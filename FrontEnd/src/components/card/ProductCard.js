...
const ProductCard = ({ products }) => {
  ...
  return (
    <React.Fragment>
      <div className='m-2 py-3'>  {products && products.ratings && products.ratings.length > 0 ? showAverage(products) : <span className="text-danger">No Ratings yet</span>}</div>
      <div className="card rounded-5 border-0 shadow">
        ...
        <div class="card-body">
          ...
          <div className='d-flex justify-content-around mt-4 py-2'>
            <Link to={`/product/${slug}`} className="text-warning">View</Link>
            <button className=" bg-white border-0 text-danger" onClick={handleToCart} disabled={products.quantity < 1}>{products.quantity < 1 ? "Out Of Stock" : "Add to Cart"}</button>
          </div>
          ...
          <div className='text-center d-flex justify-content-around'>
            ...
            <div className='fs-5'>Color
              <div class="border border-1 mx-3 rounded-circle mt-3" style={{ backgroundColor: `${color}`, height: '1.5rem', width: '1.5rem' }}></div>
            </div>
