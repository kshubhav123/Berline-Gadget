...
const ForgotPassword = () => {
    ...
    const loginForm = () => {
        return (
            <form onSubmit={handleSubmit}>
                <div className="mb-5">
                    <label className="form-label fs-5 ms-1">Email</label>
                    <input type="email" value={email} className="form-control" onChange={(e) => setEmail(e.target.value)} placeholder="Enter Your Email" />
                    <button type="submit" className="btn btn-outline-dark my-3 mb-5" disabled={!email}>Forgot Password</button>
                </div>
            </form>
        )
    }
    ...
    return (
        <React.Fragment>
            <h2 className="h2 text-center mt-5 fw-bold">Forgot Password</h2>
